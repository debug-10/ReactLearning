...
const countReducer = (state, action) => {
  ...
  switch (action.type) {
    case "increment": // 如果 action 类型是 "increment"，状态的 count 值加 1
      return { count: state.count + 1 };
    case "decrement": // 如果 action 类型是 "decrement"，状态的 count 值减 1
      return { count: state.count - 1 };
...
const Counter = () => {
  // 使用 useReducer Hook，传入 reducer 函数和初始状态 { count: 0 }，
